...
pub fn exp_vec(a: &[Scalar], b: &[RistrettoPoint]) -> Vec<RistrettoPoint> {
    if a.len() != b.len() {
        // throw some error
        //println!("lengths of vectors don't match for vector addition");
    }
    let mut out = vec![RISTRETTO_BASEPOINT_POINT; b.len()];
    for i in 0..a.len() {
        out[i] = a[i] * b[i]
    }
    out
...
mod tests {
    ...
    fn invert() {
        ...
        assert_eq!(inv[0], Scalar::from(123u64).invert());
        assert_eq!(inv[1], Scalar::from(654u64).invert());
    }
    ...
    #[test]
    fn test_polynomials() {
        let p1 = VecPoly1(vec![Scalar::from(2u8), Scalar::from(3u8)], vec![Scalar::from(5u8), Scalar::from(7u8)]);
        let p2 = VecPoly1(vec![Scalar::from(3u8), Scalar::from(5u8)], vec![Scalar::from(6u8), Scalar::from(4u8)]);

        assert_eq!(p1.eval(Scalar::from(7u8)), vec![Scalar::from(37u32),Scalar::from(52u32)]);

        let comb = p1.inner_product(&p2);
        assert_eq!(comb.0,Scalar::from(21u32));
        assert_eq!(comb.1,Scalar::from(74u32));
        assert_eq!(comb.2,Scalar::from(58u32));
    }
