...
pub struct ClientServiceHandle {
    command_tx: mpsc::Sender<Command>,
    pub(crate) channels: Arc<AtomicImmut<HashMap<SocketAddr, ChannelHandle>>>, // this just keepts track of stuff deleting it desnt remove the friendships
...
impl ClientServiceHandle {
    ...
    /// Returns the channels of the client service.
    pub fn channels(&self) -> &Arc<AtomicImmut<HashMap<SocketAddr, ChannelHandle>>> {
        &self.channels
    }
    ...
    pub fn delete_friends(&mut self) {
        ...
        for x in x.iter() {
            // let mut a = x.1.clone();
            // a.is_server_down = Arc::new(AtomicBool::new(true));
            // y.insert(x.0.clone(), a.clone());
